...
case class Node(messages: List[MessageType], answers: Option[List[Answer]]) {

  def toMessageList: List[Option[Message]] =
    (messages.reverse.head.toMessage(answers.map(list => list.map(_.quickReply))) :: messages.reverse.tail.map(_.toMessage(None))).reverse

  def getNext(payload: String): Option[Node] = Node.searchTree(answers, payload)
}

object Node {

  def containsPayload(quickReply: QuickReply, payload: String): Boolean = quickReply match {
    case text @ Text(_, _, _, _) => text.payload == payload
    case _ => false // TODO: complete
  }

  def searchDown(answer: Answer, payload: String): Option[Node] = answer match {
    case Answer(quickReply, None) => None
